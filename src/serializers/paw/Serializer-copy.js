...
export default class PawSerializer {
  ...
  import(context, items, options) {
    ...
    if (typeof parsePromiseOrResult.then !== 'function') {
      const value = parsePromiseOrResult
    ...
    const environment = new PawEnvironment()
    const resolver = new ContextResolver(environment)

    const importPromise = parsePromiseOrResult.then((requestContexts) => {
      const promises = []
      for (const env of requestContexts) {
        promises.push(
                    this._importContext(
                        resolver,
                        env.context,
  ...
  _importPawRequests(requestContext, item, options) {
    ...
    const manageRequestGroups = (current, parentGroup) => {
      if (
                !parentGroup ||
      ...
      const pawGroup = this.context.createRequestGroup(current)
  ...
  _importReferences(references) {
    const environmentDomain = this._getEnvironmentDomain()

    const variablesDict = {}

    const environments = references.keySeq()

    for (const env of environments) {
      const container = references.get(env)

      const pawEnv = this._getEnvironment(
                environmentDomain,
                container.get('name') || container.get('id') || env
            )
      const uris = container.get('cache').keySeq()
      for (const uri of uris) {
        const reference = container.resolve(uri)
        ...
        if (!(reference instanceof LateResolutionReference) ||
          ...
          const content = this._setReference(reference)
          let ds
  ...
  _setLateResolutionReference(reference) {
    ...
    const ref = (reference.get('relative') || reference.get('uri') || '')
            .slice(12)
    const match = ref.match(/({{[^{}]*}})/g)
    if (match && ref !== match[0]) {
      ...
      while ((m = re.exec(ref)) !== null) {
        ...
        const envVariable = this._getEnvironmentVariable(
  ...
  _getEnvironmentVariable(name) {
    const domain = this._getEnvironmentDomain()
    let variable = domain.getVariableByName(name)
    if (typeof variable === 'undefined') {
      const env = this._getEnvironment(domain)
      const varD = {}
      varD[name] = ''
  ...
  _applyFuncOverGroupTree(requests, group, leafFunc, nodeFunc, pawGroup) {
    let calls = []
    const currentPawGroup = nodeFunc(group.get('name') || '', pawGroup)
  ...
  _generateUrl(url, queries, auths) {
    const protocol = this._formatProtocolParam(url.get('protocol'))
    const host = this._toDynamicString(url.get('host'), true, 'host')
    const path = this._toDynamicString(url.get('pathname'), true, 'pathname')
    const hash = this._toDynamicString(url.get('hash'), true, 'url')
    ...
    const queryParams = (queries || []).concat(
            this._extractQueryParamsFromAuth(auths)
        )
    if (queryParams.size > 0) {
      _url.appendString('?')
      const _params = queryParams.reduce(
                (params, keyValue) => {
                  const dynKey = this._toDynamicString(
                  ...
                  const dynValue = this._toDynamicString(
                        keyValue, true, 'query'
                    ).components.map((component) => {
                      if (typeof component === 'string') {
                        return encodeURI(component)
  ...
  _formatProtocolParam(protocol) {
    ...
    const schema = protocol.getJSONSchema(false)
  ...
  _setAuth(pawReq, auths) {
    ...
    for (const auth of auths) {
      const rule = authTypeMap[auth.constructor.name]
  ...
  _setFormDataBody(pawReq, body) {
    ...
    const keyValues = body.map(param => {
      const key = this._toDynamicString(
                param.get('key'), true, 'body'
            )
      const value = this._toDynamicString(
                param.get, true, 'body'
            )
  ...
  _setJSONBody(pawReq, body) {
    ...
    const param = body.get(0)
    const content = this._toDynamicString(param, true, 'body')

    const component = content.getComponentAtIndex(0)
  ...
  _toDynamicString(string, defaultToEmpty, source) {
    ...
    for (const component of envComponents) {
      if (typeof component !== 'string') {
        ...
        const splits = component.match(re)
        if (splits.length > 0 && splits.length < 50) {
          const isRegular = /([^\x00-\x1f]+)/.test(splits[0])
          const splitMapper = splitManager(isRegular)
  ...
  _escapeSequenceDynamicValue(seq) {
    const escapeSequence = this._escapeCharSequence(seq)
  ...
  _castReferenceToDynamicString(reference) {
    const dv = this._extractReferenceComponent(reference)
