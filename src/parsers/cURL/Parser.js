...
export default class CurlParser {
  ...
  static detect(content) {
    ...
    const hasCurl = content.match(/curl\s/)
    if (hasCurl) {
      detection.score = 1
      return [ detection ]
    }
  ...
  _popArg() {
    if (this.idx < this.args.count()) {
      const arg = this.args.get(this.idx)
      this.idx += 1
      return arg
    }
    return null
  ...
  _parseCurlCommand() {
    ...
    while ((arg = this._popArg()) !== null) {
      if (arg.toLowerCase() === 'curl') {
        const container = this._createParameterContainer(
        ...
        const out = this._parseMultipartFormData(headers, body, method)
        headers = out[0]
        body = out[1]
        method = out[2]
      }
      else if (arg === '--form-string') {
        const out = this._parseMultipartFormString(headers, body, method)
        headers = out[0]
        body = out[1]
        method = out[2]
      }
        ...
        const out = this._parseUrlEncodedData(
        ...
        const parsed = this._parseUrl(arg, auth, queries)
    ...
    const update = this._updateRequest(urlEncodeFlag, headers, body)
    headers = update[0]
    body = update[1]
  ...
  _createRequest(_url, method, _container, _auth, timeout) {
    ...
    let name = null
    if (url) {
      const search = url.generateParam('search')
      if (search) {
  ...
  _parseUrl(url, _auth, _queries) {
    ...
    const username = _url.generateParam('username')
    if (username) {
    ...
    const password = _url.generateParam('password')
    if (password) {
  ...
  _parseQueries(search, queries) {
    const components = search.slice(1).split('&')
    return queries.concat(components.map(component => {
      ...
      return this._formatParameter(key, finalValue)
    }))
  }
  ...
  _parseCompressed(headers) {
    const header = headers.get('Accept-Encoding') || null
    let acceptEncoding
    if (!header) {
      acceptEncoding = ''
    }
