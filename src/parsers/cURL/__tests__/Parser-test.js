...
export class TestCurlParser extends UnitTest {
  ...
  testDetectCurl() {
    ...
    const input = 'curl http:httpbin.org/get -X GET'

    const expected = [ { format: 'curl', version: 'v1', score: 1 } ]
  ...
  testDetectNotACurl() {
    ...
    const input = 'not a c.u.r.l file'

    const expected = [ { format: 'curl', version: 'v1', score: 0 } ]
    const result = parser.detect(input)
  ...
  __testRequests(
    ...
    const groupMap = context.getIn([ 'group', 'children' ])
    const requestMap = context.get('requests')
