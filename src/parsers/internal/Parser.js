...
export default class InternalParser {
  ...
  parse(item) {
    ...
    const context = this._extract(obj)
  ...
  _extractContext(obj) {
    ...
    if (obj.references) {
      const refs = Object.keys(obj.references || {})
    ...
    if (obj.requests) {
      const reqs = Object.keys(obj.requests || {})
  ...
  _extractGroup(obj) {
    ...
    if (obj.children) {
      const refs = Object.keys(obj.children || {})
      for (const ref of refs) {
  ...
  _extractParameterContainer(_obj) {
    ...
    const container = new ParameterContainer(obj)
    return container
  }
  ...
  _extractReferenceContainer(_obj) {
    ...
    const cache = {}
    if (obj.cache) {
      const keys = Object.keys(obj.cache)
      for (const key of keys) {
  ...
  _extractReferenceCache(_obj) {
    ...
    if (obj.resolved) {
      const resolved = {}
      const keys = Object.keys(obj.resolved)
      for (const key of keys) {
