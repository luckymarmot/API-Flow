...
export class TestRAMLParser extends UnitTest {
  testConstructor() {
    ...
    const parser = new RAMLParser(items)
  ...
  testParseWithCustomReader(done) {
    ...
    const parser = new RAMLParser(items)
    const mockedParser = new ClassMock(parser, '')
    ...
    const promise = parser.parse.apply(
            mockedParser,
            [ item ]
        )
  ...
  testCreateGroupTreeWithResources() {
    const raml = this.__loadRAMLObject('jukebox-api')
  ...
  testCreateGroupTreeCallsCreateRequestsWithCorrectURLs() {
    ...
    for (const call of mockedParser.spy._createRequest.calls) {
  ...
  testCreateRequestReturnsARequest() {
    ...
    const request = parser._createRequest(raml, req, url, method)
  ...
  testFindReferences() {
    ...
    const result = mockedParser._findReferences(raml)
  ...
  testReplaceReferences() {
    ...
    const result = mockedParser._replaceReferences(raml)
  ...
  testCreateJSONSchemaReferenceWithFileReference() {
    ...
    const rel = '::fileRef::somefile.json'
    const item = new Item({
      file: {
        path: '/some/path',
        name: 'base.raml'
    ...
    const result = parser._createJSONSchemaReference(rel, item)
  ...
  testDetectWithRAMLFile() {
    ...
    const input = '#%RAML 0.8' +
            '\nSomeContent:.....'

    const expected = [ { format: 'raml', version: 'v0.8', score: 1 } ]
  ...
  testDetectWithNotARAMLFile() {
    ...
    const input = 'SomeContent:.....'

    const expected = [ { format: 'raml', version: 'v0.8', score: 0 } ]
    const result = parser.detect(input)
  ...
  __init(file) {
    const raml = this.__loadRAMLObject(file)
    const parser = new RAMLParser()
    const mockedParser = new ClassMock(parser, '')
