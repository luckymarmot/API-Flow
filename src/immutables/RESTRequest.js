...
import { BasicAuth, DigestAuth, NTLMAuth, NegotiateAuth, ApiKeyAuth, OAuth1Auth, OAuth2Auth } from './Auth'
...
export class Schema extends Immutable.Record({
  ...
  */
      ...
      for(let prop in schema){
        if(schema.hasOwnProperty(prop)){
          if(prop === '$ref'){
            ...
            subSchema = subSchema
              .set('uri', this.get('uri') + '/' + this._escapeURIFragment(prop))
  ...
  resolveReferences(depth=0, baseSchema=this){
    ...
    subSchema.forEach((value, key) => {
      if(value instanceof SchemaReference){
        value = value.resolve(baseSchema)
        if(depth > 0){
          let schema = value.getResolvedSchema().resolveReferences(depth-1, baseSchema)
          value = value.setResolvedSchema(schema)
        }
      }
      else {
        value = value.resolveReferences(depth, baseSchema)
      }
      auxSchema = auxSchema.set(key, value)
    })
  ...
  getInSchema(array, notSetValue){
    let path = array.reduce((path, elt) => {
      return path.concat(['map', elt])
    }, [])
