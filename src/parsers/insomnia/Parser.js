 ...
 * tries to extract a title from a RAML file
 * @param {string} content: the file to get the api title from
 * @returns {string|null} the title, if it was found
 */
 ...
 * @param {object} request - a part of the file that is relevant to the extraction of a
 * `name` field.
 * @returns {{key: 'name', value: string}|null} the corresponding group field, as a key-value
 * pair
 */
methods.extractRequestNameFromItem = (request) => {
  if (request.name || request.url) {
    return { key: 'name', value: request.name || request.url }
  }
  else {
    return null
  }
 ...
 * key-value pair
 */
methods.extractRequestDescriptionFromItem = (request) => {
  if (request.description) {
    return { key: 'description', value: request.description }
  }
  else {
    return null
  }
 ...
 * @param {object[]} parameters - a part of the file that is relevant to the extraction of a
 * `queries` field.
 * @returns {{key: 'queries', value: OrderedMap<string, Parameter|Reference>}|null} the
 * corresponding queries field, as a key-value pair
 *
 * Note that the queries block should be an OrderedMap of ids to either Parameter or Reference to
 * shared Parameters.
 */
methods.extractQueryBlockFromQueryParams = (parameters) => {
  if (!parameters || !parameters.length) {
    return null
  }

  const block = parameters.map(methods.extractParameterEntryFromQueryParameter)
    .filter(v => !!v)
    .reduce(convertEntryListInMap, {})
...
methods.extractParameterEntryFromQueryParameter = ({ name, value, disabled }) => {
  if (!name || disabled) {
    return null
  }

  const $value = methods.helperExtractParameterFromString(value)
 ...
 * @param {object[]} headers - a part of the file that is relevant to the extraction of a
 * `headers` field.
 * @returns {{key: 'headers', value: OrderedMap<string, Parameter|Reference>}|null} the
 * corresponding headers field, as a key-value pair
 *
...
methods.extractHeaderBlockFromHeaders = (headers) => {
  ...
  const block = headers.map(methods.extractParameterEntryFromHeader)
    .filter(v => !!v)
    .reduce(convertEntryListInMap, {})
...
methods.extractParameterEntryFromHeader = ({ name, value, disabled }) => {
  if (!name || disabled) {
    return null
  }
 ...
 * @param {object} body - a part of the file that is relevant to the extraction of a
 * `body` field.
 * @returns {{key: 'body', value: OrderedMap<string, Parameter|Reference>}|null} the
 * corresponding body field, as a key-value pair
 *
...
methods.extractBodyBlockFromBody = (body) => {
  if (body && body.mimeType === 'multipart/form-data') {
    return methods.extractBodyBlockFromFormDataBody(body)
  }

  if (body && body.mimeType === 'application/x-www-form-urlencoded') {
    return methods.extractBodyBlockFromUrlEncodedBody(body)
  }
...
methods.extractParameterFromFormParam = ({ name, value, disabled }) => {
  if (!name || disabled) {
    return null
  }
...
methods.extractBodyBlockFromFormDataBody = (body) => {
  if (!body) {
    return null
  }

  const block = (body.params || [])
    .map(methods.extractParameterFromFormParam)
    .filter(v => !!v)
    .reduce(convertEntryListInMap, {})
...
methods.extractBodyBlockFromUrlEncodedBody = (body) => {
  if (!body) {
    return null
  }
...
methods.extractBodyBlockFromFileBody = (body) => {
  ...
  const block = {
    file: new Parameter({ type: 'string', default: body.fileName })
  }
...
methods.extractBodyBlockFromRawBody = (body) => {
  ...
  const block = {
    raw: new Parameter({ type: 'string', default: body.text || '' })
  }
 ...
 * @returns {ParameterContainer} an object holding all the information necessary to the
 * instantiation of a ParameterContainer Record.
 */
methods.extractRequestParameterContainerInstanceFromItem = (request) => {
  const kvs = [
    methods.extractQueryBlockFromQueryParams(request.parameters),
    methods.extractHeaderBlockFromHeaders(request.headers),
    methods.extractBodyBlockFromBody(request.body)
  ].filter(v => !!v)
 ...
 */
methods.extractRequestParametersFromItem = (request) => {
 ...
 * @param {object} authentication - a part of the file that is relevant to the
 * extraction of a Reference.
 * @returns {Reference|null} the corresponding auth References
 *
 * Note that the References uuid should be the uniquely identifying values used to saved shared
 * Auths in the Store.
 */
methods.extractAuthRefFromAuth = (authentication) => {
 ...
 * @param {object} request - a part of the file that is relevant to the extraction of an
 * `auths` field.
 * @returns {{key: 'auths', value: List<Reference?>}|null} the corresponding auths field, as
 * a key-value pair
 *
 ...
 */
methods.extractAuthsFromItem = (request) => {
  const authentication = request.authentication

  if (authentication && authentication.type === 'basic') {
    return methods.extractAuthRefFromBasicAuth(authentication)
  }

  if (authentication && authentication.type === 'oauth2') {
    return methods.extractAuthRefFromOAuth2Auth(authentication)
  }

  if (authentication && authentication.type === 'digest') {
    return methods.extractAuthRefFromDigestAuth(authentication)
  }

  if (authentication && authentication.type === 'ntlm') {
    return methods.extractAuthRefFromNTLMAuth(authentication)
  }

  return null
}

methods.extractAuthRefFromBasicAuth = (authentication) => {
  // TODO: Implement this
  return {
    key: 'auths',
    value: List([
      new Reference({
        type: 'auth',
        uuid: authentication.type
      })
    ])
  }
}

methods.extractAuthRefFromOAuth2Auth = (authentication) => {
  // TODO: Implement this
  return {
    key: 'auths',
    value: List([
...
methods.extractAuthRefFromDigestAuth = (authentication) => {
  // TODO: Implement this
  return {
    key: 'auths',
    value: List([
...
methods.extractAuthRefFromNTLMAuth = (authentication) => {
  // TODO: Implement this
  return {
    key: 'auths',
    value: List([
 ...
 * a key-value pair
 */
methods.extractRequestMethodFromItem = (request) => {
  if (request.method) {
    return { key: 'method', value: request.method.toUpperCase() }
  }
  else {
    return { key: 'method', value: 'GET' }
  }
}
 ...
 * the OrderedMap.
 */
methods.extractRequestEndpointsFromItem = (request) => {
  return {
    key: 'endpoints',
    value: OrderedMap({ [request.url]: new URL(request.url) })
  }
}
 ...
 * @param {object} request - a part of the file that is relevant to the extraction of a
 * Request Record.
 * @returns {Request} an object holding all the information necessary to the
 * instantiation of a Request Record.
 */
methods.extractRequestInstanceFromItem = (request) => {
  const kvs = [
    methods.extractRequestEndpointsFromItem(request),
    methods.extractRequestNameFromItem(request),
    methods.extractRequestDescriptionFromItem(request),
 ...
 */
methods.extractRequestFromItem = (request) => {
  const key = request.method || 'GET'
  const requestInstance = methods.extractRequestInstanceFromItem(request)
...
/**
 * Convert a template string to Parameter.
 * Example: "{{ foo }} hi {{ bar }}" => Parameter with three children
 * @param {string} text - template string to convert
 * @param {string} [key] - optional key for the parameter
 * @returns {Parameter} parameter that represents the template
 */
methods.helperExtractParameterFromString = (text, key) => {
  const segments = methods.helperConvertTemplateStringToSegments(text)
  const parameters = []

  for (const segment of segments) {
    parameters.push(new Parameter(methods.helperExtractParameterArgsFromSegment(segment)))
  }
...
/**
 * Split a template string into individual segments of either variables
 * or raw strings.
 * @param {string} text - the template string to split into segments
 * @returns {Array.<*>} the segments
 */
methods.helperConvertTemplateStringToSegments = (text) => {
  const separator = '!@#$%'
  const withSeparators = text.replace(/({{[^}]*}})(?!$)/g, `${separator}$1${separator}`)
  const segments = withSeparators.split(separator)
  const withoutEmpties = segments.filter(v => v !== '')
  return withoutEmpties
}

methods.helperExtractParameterArgsFromSegment = (segment) => {
  ...
  if (match) {
    return {
      ...
      name: match[1],
      constraints: List([
        new Constraint.JSONSchema({ $ref: '#/definitions/' + match[1] })
      ])
    }
  }
  else {
    return { type: 'string', default: trimmed }
  }
}
