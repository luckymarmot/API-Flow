...
export class TestSwaggerSerializer extends UnitTest {
  ...
  testFormatPathsWithMultipleRequests() {
    ...
    parser.spyOn('_formatRequest', (_context, req) => {
      ...
      const securityDefs = {}
      const def = req.getIn([ 'url', 'host' ]).generate(true)
      securityDefs[def] = {
  ...
  testUnescapeURIFragment() {
    ...
    for (let i = 0; i < fragments.length; i += 1) {
      const result = parser._unescapeURIFragment(fragments[i])
  ...
  __init(prefix = '') {
    const parser = new SwaggerSerializer()
    const mockedParser = new ClassMock(parser, prefix)
