/**
 * A Serializer to convert Api Records into Insomnia export format.
 */
...
export class InsomniaSerializer {
  static __meta__ = __meta__

  /**
   * serializes an Api into a Insomnia export formatted string
   * @param {Api} api: the api to convert
   * @returns {string} the corresponding Insomnia collection, as a string
   */
  ...
  /**
   * returns a quality score for a content string wrt. to the export format.
...
methods.createRequestBody = (api, resource, request) => {
  return { key: 'body', value: 'TODO' }
}

methods.createRequestParameters = (api, resource, request) => {
  return { key: 'parameters', value: 'TODO' }
}

methods.createRequestMethod = (api, resource, request) => {
...
methods.createRequestHeaders = (api, resource, request) => {
  ...
  return { key: 'headers', value: headers.valueSeq().toJS() }
}

methods.createRequestAuthentication = (api, resource, request) => {
  return { key: 'authentication', value: 'TODO' }
}
...
methods.createItemFromRequest = (api, resource, request) => {
  const kvs = [
    { key: '_type', value: 'request' },
    { key: '_id', value: request.id },
    // TODO: Send parentId into this
    { key: 'parentId', value: 'TODO' },
    methods.createItemName(request),
    methods.createItemDescription(request),
    methods.createRequestUrl(api, resource, request),
    methods.createRequestMethod(request),
    methods.createRequestBody(api, request),
    methods.createRequestAuthentication(api, request),
    methods.createRequestHeaders(api, request),
    methods.createRequestParameters(api, request)
  ].filter(v => !!v)
...
methods.createRequestGroupFromResource = (api, id) => {
  const resource = api.getIn([ 'resources', id ])
  ...
  const requestGroupKVS = [
    methods.createItemNameFromResource(resource),
    methods.createItemDescription(resource)
  ].filter(v => !!v)

  const requests = methods.createItemsFromResource(api, resource)

  const requestGroup = requestGroupKVS.reduce(convertEntryListInMap, {})
...
methods.createRequestResources = (api) => {
  ...
  const items = api.get('resources')
    .map((_, id) => methods.createRequestGroupFromResource(api, id))
    .filter(v => !!v)
 ...
 * @returns {Entry<string, InsomniaExport>} the corresponding entry
 */
methods.createInsomniaExport = (api) => {
  return {
    _type: 'export',
    __export_format: 3,
    __export_date: (new Date()).toISOString(),
    __export_source: `api-flow:v${packageJSON.version}`,
    resources: [
      ...methods.createRequestResources(api)
    ]
  }
...
/**
 * serializes an Api into a Insomnia formatted string
 * @param {Api} api: the api to convert
 * @returns {string} the corresponding Insomnia object, as a string
 */
methods.serialize = ({ api }) => {
  try {
    const exportData = methods.createInsomniaExport(api)
    const serialized = JSON.stringify(exportData, null, '\t')
    return serialized
  }
  catch (e) {
    throw e
...
export default InsomniaSerializer
