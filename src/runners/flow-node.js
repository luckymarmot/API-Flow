...
export default class FlowCLI extends BaseFlow {
  ...
  load(input, opts) {
    ...
    if (base === 'raw') {
      ...
      const _path = path.resolve('./', input)
  ...
  processArguments(parser) {
    ...
    if (args.config) {
      try {
        const baseData = fs.readFileSync(args.config).toString()
    ...
    if (args.ref || args.param) {
      ...
      const params = (args.param || []).map(param => {
        return {
          key: param[0],
          value: param[1]
        }
  ...
  run(_input, _options, _callback) {
    const callback = _callback || ((result) => { return result })
    ...
    const actor = actionMap[this.action]
    if (!actor) {
